...
For the repository-pinned Supabase CLI `2.113.0`, the workflow records table-level Auth dump coverage for `auth.users`, `auth.identities` and `auth.mfa_factors` before restore. The raw `data.sql` and raw `auth_data.sql` remain unchanged in the encrypted bundle. During disposable restore only, `scripts/phase1a-restore-prepare.mjs` derives a temporary `auth_data.restore.sql` so each required Auth table has exactly one restore authority:

- if the standard `data.sql` contains a required Auth table, `data.sql` is authoritative for that table;
- if the standard `data.sql` lacks a required Auth table but explicit `auth_data.sql` contains it, the helper copies only that table's isolated `COPY`/`INSERT` block into `auth_data.restore.sql`;
- if neither dump contains a required Auth table, the restore drill fails closed.

The helper does not claim behavior for any Supabase CLI version other than the pinned version observed by the workflow run. It does not modify the encrypted backup's raw SQL files or manifest checksums.
...
4. derive runner-temp restore files `roles.restore.sql` and `auth_data.restore.sql` while preserving raw bundled SQL/checksums;
5. restore `roles.restore.sql`, `auth_schema.sql`, `migration_history_schema.sql` and `schema.sql`;
6. set `session_replication_role = replica` for trigger-safe data import;
7. restore `auth_data.restore.sql`, `data.sql` and `migration_history_data.sql`;
8. verify representative schema objects, Auth identity/MFA consistency, Production migration-history match, DD-011B/DD-012C functions, RLS/ACL posture and row counts;
9. write a non-sensitive restore summary with elapsed restore seconds;
10. remove decrypted SQL and drop the disposable restore database.
...
`auth_schema.sql` is restored before the normal public schema because DeeDou public tables, policies and functions may depend on restored Auth objects. Data import still loads any derived explicit Auth data before public data. When pinned CLI evidence shows required Auth rows already exist in the standard `data.sql`, the derived explicit Auth file is intentionally empty for those tables so Auth rows are not replayed twice.

The raw `roles.sql` generated by `supabase db dump --role-only` also remains unchanged in the encrypted bundle. For disposable local restore only, `scripts/phase1a-restore-prepare.mjs` may derive `roles.restore.sql` that suppresses narrowly allowlisted Supabase-managed role settings proven incompatible with the local verification target. The first supported case is the managed `authenticator.log_min_messages=fatal` setting, which local restore cannot apply. Any unexpected role/GUC/value pattern fails closed instead of being ignored, and the derived file is kept only in runner temp storage.
