...
-- Backend-session enforcement is staged. It remains false during migration rollout,
-- then is enabled after the sole Owner has bootstrapped a HttpOnly device session.
...
begin
  ...
  if v_context.ok is distinct from true then
    return query select false, coalesce(v_context.reason, 'DEVICE_UNREGISTERED'),
      coalesce(v_context.staff_profile_id, ''), p_location_id,
      coalesce(v_context.device_id, ''), coalesce(v_context.workstation_mode, '');
    return;
  end if;
...
-- Existing browser-callable security mutations remain for compatibility, but are
-- OWNER + AAL2 only and no longer return plaintext device secrets.
...
begin
  ...
  if not exists (
    ...
    where staff_profile_id = p_target_staff_profile_id
      and location_id = p_location_id
  ) then
    return query select false, 'TARGET_LOCATION_DENIED';
    return;
  end if;

  if not exists (select 1 from public.roles where id = p_role_id) then
    return query select false, 'ROLE_NOT_FOUND';
    return;
  end if;
  ...
  perform public.dd008c_write_audit(
    p_location_id, 'STAFF', v_owner.staff_profile_id, v_owner.staff_profile_id,
    v_owner.device_id, 'assign_staff_role_at_location', 'staff_role',
    p_target_staff_profile_id || ':' || p_role_id, 'ASSIGNED',
...
begin
  ...
  if p_role_id = 'OWNER' or public.dd011b_target_is_owner(p_target_staff_profile_id) then
    return query select false, 'SOLE_OWNER_PROTECTED';
    return;
  end if;

  update public.staff_role_assignments
  set active = false, assigned_at = now()
  where staff_profile_id = p_target_staff_profile_id
    and location_id = p_location_id
  ...
  perform public.dd008c_write_audit(
    p_location_id, 'STAFF', v_owner.staff_profile_id, v_owner.staff_profile_id,
    v_owner.device_id, 'dd011_revoke_staff_role_at_location', 'staff_role',
    p_target_staff_profile_id || ':' || p_role_id, 'REVOKED',
    jsonb_build_object('roleId', p_role_id, 'ownerAuthoritative', true)
  );
...
begin
  ...
  if p_active = false and public.dd011b_target_is_owner(p_target_staff_profile_id) then
    return query select false, 'SOLE_OWNER_PROTECTED';
    return;
  end if;
  ...
  perform public.dd008c_write_audit(
    p_location_id, 'STAFF', v_owner.staff_profile_id, v_owner.staff_profile_id,
    v_owner.device_id, 'dd011_set_staff_active', 'staff_profile', p_target_staff_profile_id,
...
begin
  ...
  if not exists (select 1 from public.staff_profiles where id = p_target_staff_profile_id) then
    return query select false, 'TARGET_STAFF_NOT_FOUND';
    return;
  end if;
  ...
  set active = excluded.active, assigned_at = now();

  if p_active = false then
    select auth_user_id into v_target_auth from public.staff_profiles where id = p_target_staff_profile_id;
    update public.workstation_device_sessions
    set active = false, revoked_at = now()
    where auth_user_id = v_target_auth and active = true;
  end if;
  ...
  perform public.dd008c_write_audit(
    p_location_id, 'STAFF', v_owner.staff_profile_id, v_owner.staff_profile_id,
    v_owner.device_id, 'dd011_set_staff_location_active', 'staff_profile', p_target_staff_profile_id,
...
begin
  ...
  if v_owner.ok is distinct from true then
    return query select false, v_owner.reason, ''::text, ''::text;
    return;
  end if;
  ...
  insert into public.workstation_devices(
    ...
    v_device_id, p_location_id, coalesce(nullif(btrim(p_label), ''), p_mode),
    p_mode, public.hash_device_credential(v_credential), true, v_owner.staff_profile_id
  );
  ...
  perform public.dd008c_write_audit(
    p_location_id, 'STAFF', v_owner.staff_profile_id, v_owner.staff_profile_id,
    v_owner.device_id, 'register_workstation_device', 'workstation_device', v_device_id,
    'REGISTERED', jsonb_build_object('mode', p_mode, 'backendManaged', true)
  );

  -- Plaintext remains backend-only; authenticated callers never receive it.
  return query select true, '', v_device_id, ''::text;
end
$$;
...
begin
  ...
  if p_device_id = v_owner.device_id then
    return query select false, 'CURRENT_DEVICE_ROTATE_BLOCKED', ''::text, ''::text;
    return;
  end if;
  ...
  set credential_hash = public.hash_device_credential(v_credential),
      active = true, rotated_at = now(), revoked_at = null,
  ...
  insert into public.workstation_device_secrets(device_id, credential, rotated_at)
  values (p_device_id, v_credential, now())
  on conflict (device_id) do update
  set credential = excluded.credential, rotated_at = now();
  ...
  perform public.dd008c_write_audit(
    p_location_id, 'STAFF', v_owner.staff_profile_id, v_owner.staff_profile_id,
    v_owner.device_id, 'dd011_rotate_workstation_device', 'workstation_device', p_device_id,
    'ROTATED', jsonb_build_object('backendManaged', true)
  );

  return query select true, '', p_device_id, ''::text;
end
$$;
...
begin
  ...
  update public.workstation_devices
  set active = false, revoked_at = now()
  where id = p_device_id and location_id = p_location_id and active = true;
  ...
  perform public.dd008c_write_audit(
    p_location_id, 'STAFF', v_owner.staff_profile_id, v_owner.staff_profile_id,
    v_owner.device_id, 'revoke_workstation_device', 'workstation_device', p_device_id,
    'REVOKED', jsonb_build_object('backendManaged', true)
  );
...
-- Service-role-only staff provisioning. Supabase Auth user creation remains in the
-- same-origin backend because auth.admin is not available to browser clients.
...
begin
  ...
  from public.staff_profiles sp
  join public.staff_role_assignments sra
    on sra.staff_profile_id = sp.id and sra.role_id = 'OWNER' and sra.active = true
  join public.staff_location_assignments sla
    on sla.staff_profile_id = sp.id and sla.location_id = p_location_id and sla.active = true
  where sp.auth_user_id = p_owner_auth_user_id and sp.active = true
  limit 1;
  ...
  insert into public.staff_profiles(
    ...
    v_staff_id, p_target_auth_user_id, v_username, btrim(p_display_name), false, 'PENDING_FIRST_LOGIN'
  );
...
declare
  ...
  v_request_id uuid;
begin
  select * into v_staff from public.staff_profiles where auth_user_id = p_auth_user_id limit 1;
  if v_staff.id is null then
    return query select false, 'STAFF_NOT_FOUND', null::uuid, ''::text, ''::text, ''::text, null::timestamptz;
    return;
  end if;

  if public.dd011b_target_is_owner(v_staff.id) then
    return query select false, 'OWNER_USES_BOOTSTRAP', null::uuid, ''::text, ''::text, ''::text, null::timestamptz;
    return;
  end if;

  if v_staff.provisioning_status = 'DISABLED' then
    return query select false, 'STAFF_DISABLED', null::uuid, ''::text, ''::text, ''::text, null::timestamptz;
    return;
  end if;
  ...
  if p_workstation_mode not in ('CASHIER','STAFF','KDS_KITCHEN','KDS_BAR','KDS_DESSERT','ADMIN')
    ...
    return query select false, 'VALIDATION_ERROR', null::uuid, ''::text, ''::text, ''::text, null::timestamptz;
    return;
  end if;
  ...
  if v_location_id is null or v_role_id is null then
    return query select false, 'STAFF_ASSIGNMENT_MISSING', null::uuid, ''::text, ''::text, ''::text, null::timestamptz;
    return;
  end if;

  v_kind := case when v_staff.provisioning_status in ('PENDING_FIRST_LOGIN','PENDING_OWNER_APPROVAL')
    then 'FIRST_LOGIN' else 'NEW_DEVICE' end;

  update public.staff_activation_requests
  set status = 'EXPIRED'
  where staff_profile_id = v_staff.id and status = 'PENDING';
...
declare
  ...
  v_owner_staff_id text;
begin
  select * into v_request from public.staff_activation_requests where id = p_request_id for update;
  ...
  join public.staff_role_assignments sra
    on sra.staff_profile_id=sp.id and sra.role_id='OWNER' and sra.active=true
  join public.staff_location_assignments sla
    on sla.staff_profile_id=sp.id and sla.location_id=v_request.location_id and sla.active=true
  where sp.auth_user_id=p_owner_auth_user_id and sp.active=true
  limit 1;
  ...
  if v_request.activation_kind = 'FIRST_LOGIN' then
    ...
    set active=true, assigned_at=now()
    where staff_profile_id=v_request.staff_profile_id and location_id=v_request.location_id;
    update public.staff_role_assignments
    set active=true, assigned_at=now()
    where staff_profile_id=v_request.staff_profile_id
      and location_id=v_request.location_id and role_id=v_request.role_id;
  end if;

  update public.staff_activation_requests
  set status='APPROVED', approved_at=now(), approved_by_staff_profile_id=v_owner_staff_id
  ...
  perform public.dd008c_write_audit(
    ...
    'APPROVED', jsonb_build_object('staffProfileId', v_request.staff_profile_id, 'kind', v_request.activation_kind)
  );
...
begin
  ...
  select sp.id into v_owner_staff_id
  from public.staff_profiles sp
  join public.staff_role_assignments sra on sra.staff_profile_id=sp.id and sra.role_id='OWNER' and sra.active=true
  where sp.auth_user_id=p_owner_auth_user_id and sp.active=true
  limit 1;
  if v_owner_staff_id is null then return query select false,'OWNER_REQUIRED'; return; end if;

  update public.staff_activation_requests set status='REJECTED', rejected_at=now() where id=p_request_id;
  if v_request.activation_kind='FIRST_LOGIN' then
    update public.staff_profiles set provisioning_status='PENDING_FIRST_LOGIN', updated_at=now()
    where id=v_request.staff_profile_id and active=false;
  end if;

  perform public.dd008c_write_audit(
    v_request.location_id,'STAFF',v_owner_staff_id,v_owner_staff_id,'',
    'dd011b_service_reject_activation','staff_activation',p_request_id::text,'REJECTED','{}'::jsonb
  );
  return query select true,'';
end
$$;
...
returns table(
  ok boolean, reason text, request_id uuid, status text,
  activation_kind text, expires_at timestamptz
)
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if v_request.id is null then
    return query select false,'ACTIVATION_NOT_FOUND',null::uuid,''::text,''::text,null::timestamptz;
    return;
  end if;
  ...
  return query select true,'',v_request.id,v_request.status,v_request.activation_kind,v_request.expires_at;
end
$$;
...
begin
  ...
  insert into public.workstation_devices(
    ...
    v_device_id, v_request.location_id, v_request.device_label, v_request.workstation_mode,
    public.hash_device_credential(v_credential), true, v_request.approved_by_staff_profile_id
  );
  ...
  update public.staff_activation_requests
  set status='COMPLETED',completed_at=now(),device_id=v_device_id
...
returns table(
  ok boolean, reason text, staff_profile_id text, device_id text,
  location_id text, workstation_mode text, device_credential text
)
language plpgsql
security definer
set search_path = ''
...
declare
  v_row record;
begin
  select sp.id as staff_profile_id, wd.id as device_id, wd.location_id,
         wd.mode as workstation_mode, sec.credential, s.id as session_id
  into v_row
  from public.workstation_device_sessions s
  join public.workstation_devices wd on wd.id=s.device_id and wd.active=true
  join public.workstation_device_secrets sec on sec.device_id=wd.id
  join public.staff_profiles sp on sp.auth_user_id=s.auth_user_id and sp.active=true
  join public.staff_location_assignments sla
    on sla.staff_profile_id=sp.id and sla.location_id=wd.location_id and sla.active=true
  ...
  if v_row.device_id is null then
    return query select false,'DEVICE_SESSION_REQUIRED','','','','',''; return;
  end if;

  update public.workstation_device_sessions set last_seen_at=now() where id=v_row.session_id;
  update public.workstation_devices
  set last_seen_at=now(),last_used_by_staff_profile_id=v_row.staff_profile_id,use_count=use_count+1
  where id=v_row.device_id;

  return query select true,'',v_row.staff_profile_id,v_row.device_id,v_row.location_id,v_row.workstation_mode,v_row.credential;
end
$$;

create or replace function public.dd011b_service_owner_security_snapshot(p_owner_auth_user_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_location_id text;
begin
  select sp.id,sra.location_id into v_owner_staff_id,v_location_id
  from public.staff_profiles sp
  join public.staff_role_assignments sra on sra.staff_profile_id=sp.id and sra.role_id='OWNER' and sra.active=true
  join public.staff_location_assignments sla on sla.staff_profile_id=sp.id and sla.location_id=sra.location_id and sla.active=true
  where sp.auth_user_id=p_owner_auth_user_id and sp.active=true limit 1;
  if v_owner_staff_id is null then return jsonb_build_object('ok',false,'reason','OWNER_REQUIRED'); end if;
  ...
  return jsonb_build_object(
    ...
    'staff',coalesce((
      select jsonb_agg(jsonb_build_object(
        'staffProfileId',sp.id,'username',sp.username,'displayName',sp.display_name,
        'active',sp.active,'provisioningStatus',sp.provisioning_status,
        'locationActive',sla.active,
        'roles',coalesce((select jsonb_agg(sra.role_id order by sra.role_id) from public.staff_role_assignments sra where sra.staff_profile_id=sp.id and sra.location_id=v_location_id and sra.active=true),'[]'::jsonb)
      ) order by sp.display_name,sp.id)
      from public.staff_profiles sp
      join public.staff_location_assignments sla on sla.staff_profile_id=sp.id and sla.location_id=v_location_id
    ),'[]'::jsonb),
    'pendingActivations',coalesce((
      select jsonb_agg(jsonb_build_object(
        'requestId',ar.id,'staffProfileId',ar.staff_profile_id,'username',sp.username,
        'displayName',sp.display_name,'roleId',ar.role_id,'deviceLabel',ar.device_label,
        'workstationMode',ar.workstation_mode,'verificationCode',ar.verification_code,
        'activationKind',ar.activation_kind,'status',ar.status,'requestedAt',ar.requested_at,'expiresAt',ar.expires_at
      ) order by ar.requested_at desc)
      from public.staff_activation_requests ar join public.staff_profiles sp on sp.id=ar.staff_profile_id
      where ar.location_id=v_location_id and ar.status='PENDING' and ar.expires_at>now()
    ),'[]'::jsonb),
    'devices',coalesce((
      select jsonb_agg(jsonb_build_object(
        'deviceId',wd.id,'label',wd.label,'mode',wd.mode,'active',wd.active,
        'createdAt',wd.created_at,'lastSeenAt',wd.last_seen_at,'useCount',wd.use_count,'revokedAt',wd.revoked_at
      ) order by wd.active desc,wd.created_at desc)
      from public.workstation_devices wd where wd.location_id=v_location_id
    ),'[]'::jsonb),
    'roles',coalesce((
      select jsonb_agg(jsonb_build_object('roleId',r.id,'roleName',r.name) order by r.id)
      from public.roles r where r.id<>'OWNER'
    ),'[]'::jsonb)
  );
end
$$;

create or replace function public.dd011b_service_set_device_session_enforcement(p_required boolean)
returns void
language sql
security definer
set search_path = ''
as $$
  ...
  set backend_device_sessions_required=p_required,updated_at=now()
  where singleton=true
$$;

-- Internal/service helpers are never browser-executable.
